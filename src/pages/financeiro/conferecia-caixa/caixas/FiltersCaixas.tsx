...
const FiltersCaixas = ({ refetch }: { refetch: () => void }) => {
  const filters = useStoreTableCaixas((state) => state.filters);
  const setFilters = useStoreTableCaixas((state) => state.setFilters);
  const resetFilters = useStoreTableCaixas((state) => state.resetFilters);
          ...
          <ScrollArea className="w-fill whitespace-nowrap rounded-md pb-4">
            <div className="flex w-max space-x-3">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="0">Todos os caixas</SelectItem>
                  <SelectItem value="1">Somente caixas divergentes</SelectItem>
                </SelectContent>
              </Select>
              <Select
                value={filters.nao_resolvidos}
                onValueChange={(ativo) => {
                  setFilters({ nao_resolvidos: ativo });
                }}
              >
                <SelectTrigger className="min-w-[180px] w-fit">
                ...
                <SelectContent>
                  <SelectItem value="0">Todas ocorrências</SelectItem>
                  <SelectItem value="1">Ocorrências não resolvidas</SelectItem>
                </SelectContent>
              </Select>
            </div>
