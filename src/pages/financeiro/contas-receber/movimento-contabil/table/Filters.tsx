...
const FiltersMovimentoContabiluseStoreTableMovimentoContabil = ({
  ...
  const [filters, setFilters, resetFilters] = useStoreTableMovimentoContabil((state) => [
    state.filters,
    state.setFilters,
    state.resetFilters,
  ]);

  const { mutate: downloadZip, isPending: isLoading } = useMovimentoContabilCR().downloadZip();
  const [modalContaBancariaOpen, setModalContaBancariaOpen] = useState<boolean>(false);
  const [modalGrupoEconomicoOpen, setModalGrupoEconomicoOpen] = useState<boolean>(false);
  ...
  async function getRelatorio() {
    ...
    if (!filters.range_data?.from && !filters.range_data?.to) {
      toast({
        title: "Dados insuficientes!",
        description: "Para poder gerar o relatório selecione o período de pagamento",
        variant: "destructive",
      });
      return;
    }
          ...
          {isLoading ? (
            <Button size={"xs"} disabled>
              <span className="flex gap-2 w-full items-center justify-center">
                <FaSpinner size={12} className="ms-2 animate-spin" /> Gerando Relatório...
              </span>
            </Button>
          ) : (
