...
const ModalLogImportacaoRelatorios = ({
  ...
  const [relatoriosList, setRelatoriosList] =
    useState<string[] | []>([]);
  const { data, isLoading, isError, refetch } =
    useQuery({
      queryKey: [
        "root",
        "log_import_relatorio",
        "lista",
        { pagination, relatoriosList },
      ],
      refetchOnMount: true,
      staleTime: 0,
      queryFn: async () => {
        const result = await api.get(
          "/financeiro/controle-de-caixa/importacoes/",
          {
            params: {
              pagination,
              filters: {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[1000px]">
        ...
        >
          {data?.rows.map(
            (item: LogImportacaoRelatorio) => (
              <ModalComponentRow
                key={`logImportRelatorio.${item.id}`}
              >
                <>
