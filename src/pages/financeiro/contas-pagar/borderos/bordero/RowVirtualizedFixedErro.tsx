...
interface RowVirtualizerFixedErroProps {
  ...
  form: any;
  removeItem: (
    item: RemoveItemVencimentosProps
  ) => void;
  modalEditing: boolean;
}

const RowVirtualizerFixedErro: React.FC<
  RowVirtualizerFixedErroProps
> = ({
  data,
  filteredData,
  form,
  ...
  const [openModalFatura] = useStoreCartao(
    (state) => [state.openModalFatura]
  );
  ...
  const allChecked =
    filteredData.length ===
    filteredData.filter((item) => item.checked)
      .length;
  ...
  return (
    ...
    >
      <div className="flex gap-1 font-medium text-sm w-full sticky top-0 z-10 bg-slate-200 dark:bg-blue-950 px-1">
                ...
                (filteredVencimento) => {
                  const indexData =
                    data.findIndex(
                      (vencimento) =>
                        vencimento.id_item ==
                          filteredVencimento.id_item &&
                        vencimento.tipo ==
                          filteredVencimento.tipo
                    );
        ...
        <p className="min-w-56 bg-slate-200 dark:bg-blue-950">
          Observação
        </p>
      ...
      >
        {virtualizer
          ...
          .map((item, index) => {
            ...
            const disabled = !data[indexData]
              .can_remove
              ? true
              : false;
            const tipo = form.watch(
              `itens.${indexData}.tipo_baixa`
            );
            ...
            return (
              ...
              >
                ...
                <FormSelect
                  ...
                  control={form.control}
                  disabled={
                    !modalEditing || disabled
                  }
                  options={tipoBaixa.map(
                    (
                      tipo_baixa: TipoBaixaProps
                    ) => ({
                      value:
                        tipo_baixa.id.toString(),
                      label: tipo_baixa.label,
                    })
                  )}
                  onChange={() => {
                    form.setValue(
                      `itens.${indexData}.valor_pago`,
                      valor
                    );
                    form.setValue(
                      `itens.${indexData}.updated`,
