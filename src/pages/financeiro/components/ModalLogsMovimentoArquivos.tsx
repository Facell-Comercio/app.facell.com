...
type ModalLogsMovimentoArquivosProps = {
  open: boolean;
...
const ModalLogsMovimentoArquivos = ({
  open,
  onOpenChange,
  relatorios,
}: ModalLogsMovimentoArquivosProps) => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const result = await api.get("/financeiro/controle-de-caixa/importacoes/", {
        params: { pagination, filters: { relatorios: relatoriosList } },
      });
      return result.data;
    },
  });
  ...
  useEffect(() => {
    open && setRelatoriosList(() => [...relatorios]);
  }, [open]);
      ...
      <DialogContent className="sm:max-w-[1000px]">
        ...
        >
          {data?.rows.map((item: LogImportacaoRelatorio) => (
            <ModalComponentRow key={`logImportRelatorio.${item.id}`} className="text-sm">
              <>
                ...
                <span className="w-[60ch]">
                  POR: {normalizeFirstAndLastName(item.usuario || "SISTEMA")}
                </span>
              </>
            </ModalComponentRow>
          ))}
...
export default ModalLogsMovimentoArquivos;
