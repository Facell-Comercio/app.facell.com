...
const ReactTableVirtualized: React.FC<RowVirtualizerConciliacoesProps> = ({ data }) => {
  ...
  const columns = useMemo<ColumnDef<ConciliacoesProps>[]>(
    () => [
      ...
      },
      {
        ...
        size: 100,
        cell: (info) => {
          const valor = parseFloat(info.getValue<string>()).toLocaleString("pt-BR", {
            style: "decimal",
...

      {
        ...
        size: 200,
        cell: (info) => {
          const valor = normalizeFirstAndLastName(info.getValue<string>().toUpperCase() || "");
              ...
              <table className="grid text-nowrap text-xs hover:">
                ...
                >
                  {virtualizer.getVirtualItems().map((virtualRow, index) => {
                    const row = rows[virtualRow.index] as Row<ConciliacoesProps>;
                    return (
                      <tr
