...
const FormNovoPadrao = ({
  ...
  const handleSavePadrao = async () => {
    try {
      ...
      if (
        !newPadrao?.descricao ||
        newPadrao?.descricao?.length < 1
      ) {
        throw new Error(
          "Preencha a descrição! No mínimo 1 caracter"
        );
      }
      if (
        !newPadrao?.tipo_transacao ||
        newPadrao?.tipo_transacao?.length < 1
      ) {
        throw new Error(
          "Preencha o Tipo de transação! DEBIT ou CREDIT"
        );
      }

      setSaving(true);
      await api.post(
        "/financeiro/conciliacao-bancaria/transacao-padrao",
        {
