...
const FormNovoPadrao = ({ conta }: { conta: ContaBancaria | null }) => {
    ...
    const [newPadrao, setNewPadrao] = useState<Partial<TransacaoPadrao>>(initialTranscaoPadrao)
    ...
    const handleSavePadrao = async () => {
        try {
            if (!conta?.id) {
                throw new Error('Selecione a conta bancária antes de tentar criar um padrão!')
            }
            ...
            setSaving(true)
            await api.post('/financeiro/conciliacao-bancaria/transacao-padrao', { descricao: newPadrao.descricao, tipo_transacao: newPadrao.tipo_transacao, id_conta_bancaria: conta?.id })
        ...
        {open ? (
            <div className="flex gap-3">
                <Input
                    ...
                    onChange={(e) => setNewPadrao((state) => ({ ...state, descricao: e.target.value }))}
                />
                <Select
                    value={newPadrao.tipo_transacao}
                    onValueChange={(tipo_transacao: 'CREDIT' | 'DEBIT') => setNewPadrao((state) => ({ ...state, tipo_transacao }))}
                >
                ...
                <Button disabled={saving} variant={'secondary'} onClick={handleClickCancel}>Cancelar</Button>
                <Button disabled={saving} onClick={handleSavePadrao}>Salvar</Button>
            </div>
        ) : (
