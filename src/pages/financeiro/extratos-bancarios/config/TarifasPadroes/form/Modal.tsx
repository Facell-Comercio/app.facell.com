...
const ModalTarifas = () => {
  const [id, modalOpen, closeModal, modalEditing, editModal] = useStoreTableTarifas((state) => [
    state.id,
    state.modalOpen,
    state.closeModal,
    state.modalEditing,
  ...
  const [openModalPlanoContas, setOpenModalPlanoContas] = useState<boolean>(false);
  const formRef = useRef(null);

  const { data, isSuccess, isLoading } = useTarifas().getOne(id);
  const newData: TarifaProps & Record<string, any> = {} as TarifaProps & Record<string, any>;
          ...
          {modalOpen && !isLoading && (!id || isSuccess) ? (
            <Form {...form}>
              ...
              >
                ...
                <section className="flex w-full gap-3">
                  <FormInput
                    title={!id_grupo_economico ? "Primeiro selecione o grupo econômico" : ""}
                    name="centro_custo"
