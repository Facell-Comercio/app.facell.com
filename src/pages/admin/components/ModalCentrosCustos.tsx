...
const ModalCentrosCustos = ({ open, handleSelecion, onOpenChange, closeOnSelection }: IModalCentrosCustos) => {
    const [search, setSearch] = useState<string>('')
    const [pagination, setPagination] = useState<PaginationProps>({ pageSize: 15, pageIndex: 0 })

    const { data, isLoading, isError } = useCentroCustos().getAll({pagination, filters: {termo: search}})
                ...
                <ScrollArea className="h-72 w-full rounded-md border p-3">
                    {data?.data?.rows
                        .map((item: CentroCustos) => (
                            <div key={`centroCentroCustos.${item.id}`} className="flex gap-1 items-center bg-blue-100 dark:bg-blue-700 justify-between mb-1 border rounded-md p-2">
