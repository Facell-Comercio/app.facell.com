  ...
  const handleSubmit = async () => {
    try {
      ...
      const data = await api
        .postForm("/comercial/comissionamento/configuracoes/import/tim-esteira-full", form)
        ...
        action: (
          <ToastAction
            ...
            onClick={() =>
              exportToExcel(
                data,
                `RESULTADO IMPORTAÇÃO TIM ESTEIRA FULL ${formatDate(
                  new Date(),
                  "dd-MM-yyyy hh.mm"
                )}`
              )
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      <ModalLogsMovimentoArquivos
        ...
        relatorios={["IMPORT_TIM_ESTEIRA_FULL"]}
      />
    </section>
  );
};

export default TimEsteiraFull;
