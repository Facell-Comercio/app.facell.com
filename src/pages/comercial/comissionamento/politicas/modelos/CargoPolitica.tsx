...
import Modelo, { ModeloPoliticaProps } from "./Modelo";
...
const CargoPolitica = ({ className, data }: { data: CargoProps; className?: string }) => {
  const [openModalModelo] = useStoreComissionamentoPoliticas((state) => [state.openModalModelo]);
  const [itemOpen, setItemOpen] = useState("");
  const { mutate: removeCargoPolitica } = usePoliticas().removeCargoPolitica();
  return (
    <Accordion
      type="single"
      collapsible
    ...
    >
      <AccordionItem
        value={data?.nome?.toLowerCase().replaceAll(" ", "_") || ""}
        ...
        >
          ...
          {data?.modelos?.map((modelo, index) => (
            <Modelo
              ...
              escalonamentoCargo={{
                descricao: data.escalonamento || "",
                itens: data.escalonamento_itens?.map((item) => String(item.percentual)) || [],
              }}
            />
          ))}
        </AccordionContent>
