...
export const usePermissoes = () => {
  ...
  return {
    getAll: ({ pagination, filters }: GetAllParams) =>
      useQuery({
        queryKey: ["permissao", "lista", { pagination, filters }],
    ...
    insertOne: () =>
      useMutation({
        mutationFn: async (data: Permissao) => {
          return await api.post("/permissao", data).then((response) => response.data);
        },
        onSuccess() {
    ...
    update: () =>
      useMutation({
        mutationFn: async ({ id, ...rest }: Permissao) => {
          return await api.put("/permissao", { id, ...rest }).then((response) => response.data);
        },
        onSuccess() {
          queryClient.invalidateQueries({ queryKey: ["permissao"] });
        },
