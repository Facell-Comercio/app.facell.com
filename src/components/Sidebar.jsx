...
export function Sidebar() {
  ...
  const resetItemActive = () => { setItemActive(prev => prev = null) }
      ...
      <nav className="h-full flex flex-col border-r shadow-lg ">
        <div className="p-4 pb-2 flex justify-between items-center ">
          <Link to="/" className="flex gap-2 items-center">
            <img src="src/assets/images/facell-192x192.png" className={`overflow-hidden transition-all w-16`} alt="" />
            ...
            .map((item, itemIndex) => (
              ...
              >
                <Link
                  ...
                  className={`${!sidebarOpen && "justify-center"}  ${item.type === "title" ? "text-primary" : "dark:text-white light:text-gray-300"} text-base flex items-center gap-x-4  rounded-md ${
                    item.spacing ? "mt-6" : "mt-1"
                  } ${item.type !== "title" && "hover:bg-blue-200 dark:hover:bg-blue-800 cursor-pointer"}
                ...
                >
                  ...
                  >
                    <div className={` flex gap-3 items-center`}>
                      ...
                      {!sidebarOpen && (
                            <div className={`absolute left-full rounded-md px-2 py-1 ml-6 text-nowrap bg-blue-100 text-blue-800 text-sm opacity-20 invisible -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}>{item.name}</div>
                            )}
                    ...
                    .map((subitem, subitemIndex) => (
                      <Link key={subitemIndex} to={(subitem.type === "link" && subitem.uri) || "#"}>
                        ...
                        >
                          ...
                          {!sidebarOpen && (
                            <div className={`absolute left-full rounded-md px-2 py-1 ml-6 text-nowrap bg-blue-100 text-blue-800 text-sm opacity-20 invisible -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}>{subitem.name}</div>
                            )}
                        </li>
                      </Link>
                    ))}
